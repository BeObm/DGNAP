...
def write_results(best_model, test_performances_record):
    ...
    with open(
        ...
        results.write(f'Best model {search_metric}: {float(config["results"][f"Best_model_{search_metric}"])}\n')
    ...
    print(f'Best model {search_metric}: {float(config["results"][f"Best_model_{search_metric}"])}\n')
