...
class GNN_Model(MessagePassing):
    def __init__(self, param_dict):
        ...
        if self.normalize1 != False:
            self.normalizer_function = self.normalize1(self.input_conv2)
...
def test_function(accelerator, model, test_loader, type_data="val"):
    ...
    for data in test_loader:
        ...
        # all_pred = accelerator.gather_for_metrics(pred[mask])
        # all_targets = accelerator.gather_for_metrics(data.y[mask])
        all_pred = accelerator.pad_across_processes(pred[mask])
        all_targets = accelerator.pad_across_processes(data.y[mask])
