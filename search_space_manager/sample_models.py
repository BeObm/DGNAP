...
def controlled_stratified_sampling(n_sample, e_search_space, predictor=False):
    set_seed()
    temp_dict = {}
    total_choices = 0
    for k, v in e_search_space.items():
    ...
    s = int(n_sample / total_choices) + 1
    model_list = []
...
def random_sample_backup(e_search_space, n_sample=0, predictor=False):
    timestart = time.time()
    set_seed()
    ...
    if n_sample != 0:
        ...
        for i in tqdm(range(n_sample)):
            for function, options in e_search_space.items():
                ...
                model_dict[function].append(
                    (choice, e_search_space[function][choice][0], e_search_space[function][choice][1]))
