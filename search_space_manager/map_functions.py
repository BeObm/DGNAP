...
from search_space_manager.bernstein  import *
from search_space_manager.chebnet  import *
from search_space_manager.appnp  import *
...
def map_num_graph_filters(name):
    return name
...
def map_attention(name):
    if name.lower() == 'relu6':
        ...
        return nn.Tanh()
    elif name.lower() == "multiheadattention":
...
def map_aggregation(name):
    return name

def map_dropout(name):
    return name

def map_hidden_channels(name):
    return name
...
def map_weight_decay(name):
    return name

def map_mlp_layer(name):
    return name

def map_graph_filter(name):
    if name=="bernstein":
        return Bernstein
    elif name=="B-Spline":
         return SplineConv
